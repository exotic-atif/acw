(function() {
  // Find the ACW script tag (current running script)
  const acwScript = document.currentScript || document.getElementById("acwScrpt");
  if (!acwScript) return;

  const navbarAttr = acwScript.getAttribute("data-acw-navbar");
  const darkMode = acwScript.getAttribute("data-acw-darkModeBtn") === "true";
  const coolPage = acwScript.getAttribute("data-acw-coolPage") === "true";
  ...
  if (navbarAttr && navbarAttr.toLowerCase() !== "false" && navbarAttr.trim() !== "") {
    navbarTitle = navbarAttr.toLowerCase() === "true" ? "ACW" : navbarAttr;
  }
